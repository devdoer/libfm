...
double fm_model::predict(sparse_row<FM_FLOAT>& x, DVector<double> &sum, DVector<double> &sum_sqr) {//w0+w1*X+<V,V>X.X
	double result = 0;
	if (k0) {	
		result += w0;
	}
	...
	for (int f = 0; f < num_factor; f++) {//K
		...
		result += 0.5 * (sum(f)*sum(f) - sum_sqr(f));//tip: (a+b)^2-(a^2+b^2)
	}
	return result;
}
